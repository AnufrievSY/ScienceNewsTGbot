...
def get_row(BOX):
  ...
  subject_list = [
      i for i in h.xpath('//a[contains(@href, "subject")]/text()') if i != '\n'
  ...
  NSKD = '\n'.join(
      [name, ';'.join(subject_list), ';'.join(keyword_list), description])
  ...
  ROW['Номер'] = re.sub(
      r'^\s|\s{2,}|\n]', '', ''.join(
          h.xpath('//span[@class="content-box-header-element-5"]//text()')))
...
def get_df(last_message_id=None):
  ...
  id_list = [
      re.sub(r'\n|\s', '', news_id) for news_id in html.fromstring(str(
          soup)).xpath('//*[contains(@class, "element-5")]/a/text()')
  ]
